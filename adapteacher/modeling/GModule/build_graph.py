...
class PrototypeComputation(object):
    ...
    def compute_targets_for_locations(self, locations, targets, object_sizes_of_interest):
        ...
        for im_i in range(len(targets)): 
            ...
            # Lvxg: default annotation is COCO, i.e. xyxy
            if len(targets_per_im): 
                bboxes = targets_per_im._fields['gt_boxes'].tensor.to(locations.device)
                labels_per_im = (targets_per_im._fields['gt_classes']+1).to(locations.device) # Lvxg
    ...
    def __call__(self, locations, features, targets):
        ...
        if locations: # Sampling in the source domain
            if all(len(target) for target in targets):#len(labels):
